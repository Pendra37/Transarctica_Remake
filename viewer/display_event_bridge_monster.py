...
class DisplayEventBridgeMonster(Layer):
    display_pos_size = {"display_event_bridge_monster": {"X": 0, "Y": 0.175, "W":1, "H":0.75}}
    ...
    def do_engagement(self):
        message=self.transarctica.calculate_loot_loss("bridge_monster", 15)
    ...
    def _load_background(self):
        background = Sprite(self.gallery.content["display"]["event_bridge_monster"])
        self.optimal_scale=(self.config.window_width * self.display_pos_size["display_event_bridge_monster"]["W"]) / background.width
        background.image_anchor = 0, 0
        background.scale = self.optimal_scale
        background.x = self.config.window_width * self.display_pos_size["display_event_bridge_monster"]["X"]
        background.y = self.config.window_height * self.display_pos_size["display_event_bridge_monster"]["Y"]
    ...
    def _load_interface(self):
        ...
        self.label_cityname = Label("Bridge monster attack!", (director.window.width * self.button_positions["label_cityname"]["X"], director.window.height*self.button_positions["label_cityname"]["Y"]), color=(210,200,128,255), font_name="Arial", bold=True, font_size=24, anchor_x="center", anchor_y="center")
