...
class DisplayEventMammothHerd(Layer):
    display_pos_size = {"display_event_mammoth_herd": {"X": 0, "Y": 0.175, "W":1, "H":0.75}}
    button_positions = {"label_cityname": {"X": 0.5, "Y": 0.97},
                        "button_city_table": {"X": 0.0, "Y": 0.984},
                        "button_exit": {"X": 0.88, "Y": -0.16},
                        "button_hunt": {"X": 0.78, "Y": -0.16},
    ...
    def do_engagement(self):
        self.roamer = director.core.query_mover("Roamer"+str(self.transarctica.opfor_id))
        self.transarctica_force=self.transarctica.calculate_force_value("hunt")
        self.herd_force=self.roamer.force_rating
        self.get("label_line_1").element.text="Your force: "+str(self.transarctica_force) + ", Herd force: "+str(self.herd_force)+", Livestock space available: "+str(self.transarctica.storage_cap["mammoths"]["max"]-self.transarctica.storage_cap["mammoths"]["current"])
        self._load_button_gen("button", "hunt", events.emit_mammoth_hunt, "button_hunt", 0, 1, 0.95)

    def do_mammoth_hunt(self):
        self.remove("button_hunt")
        message=self.transarctica.calculate_loot_loss("mammoth_herd", 8 * min(6,self.herd_force//self.transarctica_force))
        message=self.transarctica.calculate_loot_gain("mammoth_herd", self.transarctica_force, self.herd_force)
        ...
        for item_id in self.transarctica.cargo_manifest:
            if self.transarctica.cargo_manifest[item_id]["gain"]>0:
                ...
                if len(gain_list)>90:
                    self.get("label_line_"+str(min(4,line))).element.text=gain_list
                    gain_list=""
                    line+=1
        self.get("label_line_"+str(min(4,line))).element.text=gain_list
    ...
    def _load_background(self):
        background = Sprite(self.gallery.content["display"]["event_mammoth_herd"])
        self.optimal_scale=(self.config.window_width * self.display_pos_size["display_event_mammoth_herd"]["W"]) / background.width
        background.image_anchor = 0, 0
        background.scale = self.optimal_scale
        background.x = self.config.window_width * self.display_pos_size["display_event_mammoth_herd"]["X"]
        background.y = self.config.window_height * self.display_pos_size["display_event_mammoth_herd"]["Y"]
    ...
    def _load_interface(self):
        ...
        self.label_cityname = Label("Mammoth herd!", (director.window.width * self.button_positions["label_cityname"]["X"], director.window.height*self.button_positions["label_cityname"]["Y"]), color=(210,200,128,255), font_name="Arial", bold=True, font_size=24, anchor_x="center", anchor_y="center")
