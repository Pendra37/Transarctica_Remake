...
class DisplayEventNomads(Layer):
    display_pos_size = {"display_event_nomads": {"X": 0, "Y": 0.175, "W":1, "H":0.75}}
    ...
    def _load_background(self):
        background = Sprite(self.gallery.content["display"]["event_nomads"])
        self.optimal_scale=(self.config.window_width * self.display_pos_size["display_event_nomads"]["W"]) / background.width
        background.image_anchor = 0, 0
        background.scale = self.optimal_scale
        background.x = self.config.window_width * self.display_pos_size["display_event_nomads"]["X"]
        background.y = self.config.window_height * self.display_pos_size["display_event_nomads"]["Y"]
